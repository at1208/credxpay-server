...
module.exports.create_payout = (req, res) => {
   ...
   Payment.find({ beneficiary: beneficiary_id })
  ...
  // Creating Contact of beneficiary
   ...
   Payment.findOneAndUpdate({ beneficiary: beneficiary_id }, { razorpay_contact_id: JSON.parse(response.body).id}, { new: true })
   .exec((err, response2) => {
     if(err){
       return res.status(400).json({
         error: err
     ...
     // Creating fund account of the beneficiary
        ...
        request(options, function (error, result) {
         ...
         Payment.findOneAndUpdate({beneficiary: beneficiary_id}, {razorpay_fund_id: JSON.parse(result.body).id},{ new: true })
           .exec((err, response3) => {
             if(err){
               return res.status(400).json({
                 error: err
             ...
             .exec((err, response5) => {
               if(err){
                 ...
                 var options = {
                   ...
                   body: JSON.stringify({"account_number": process.env.RAZORPAYX_ACCOUNT_NO,"fund_account_id": JSON.parse(result.body).id,"amount":resp[0].beneficiary.amount*100,"currency":"INR","mode":response5[0].payout_mode,"purpose":"payout","queue_if_low_balance":true})
                 };
                 ...
                 request(options, function (error, data) {
                   if (error) {
                     ...
                     Payment.findOneAndUpdate({beneficiary: beneficiary_id},{razorpay_payout_id: JSON.parse(result.body).id,status: "SUCCESS"},{ new: true })
                     .exec((err, response4) => {
                         if(err){
                           return res.status(400).json({
                             error: err
